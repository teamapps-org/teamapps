...
export class CommandExecutor {
	private commandExecutorsByCommandName: {[commandName: string]: (targetObject: any, command: any) => void} = {
		    ...
		    'UiChatDisplay.addChatMessages': async (targetObject: any, command: any) => {
				return await targetObject.addChatMessages(command.chatMessages, command.prepend, command.includesFirstMessage);
			},
		    'UiChatDisplay.replaceChatMessages': async (targetObject: any, command: any) => {
				return await targetObject.replaceChatMessages(command.chatMessages, command.includesFirstMessage);
			},
		    ...
		    'UiTable.addData': async (targetObject: any, command: any) => {
				return await targetObject.addData(command.startRowIndex, command.data, command.totalNumberOfRecords, command.sortField, command.sortDirection, command.clearTableCache);
			},
		    'UiTable.removeData': async (targetObject: any, command: any) => {
				return await targetObject.removeData(command.ids);
			},
		    'UiTable.insertRows': async (targetObject: any, command: any) => {
				return await targetObject.insertRows(command.index, command.data);
			},
		    'UiTable.deleteRows': async (targetObject: any, command: any) => {
				return await targetObject.deleteRows(command.ids);
			},
		    'UiTable.updateRecord': async (targetObject: any, command: any) => {
				return await targetObject.updateRecord(command.record);
			},
		    ...
		    'UiTable.setChildrenData': async (targetObject: any, command: any) => {
				return await targetObject.setChildrenData(command.parentRecordId, command.data);
			},
		    ...
		    'UiTable.selectRows': async (targetObject: any, command: any) => {
				return await targetObject.selectRows(command.recordIds, command.scrollToFirstRecord);
			},
