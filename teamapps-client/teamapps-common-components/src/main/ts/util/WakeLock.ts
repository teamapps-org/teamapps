...
if ('wakeLock' in navigator && 'request' in (navigator as any).wakeLock) {
	...
	wakeLockApi = {
		request: () => new Promise((resolve, reject) => {
			const c = counter++;
			console.info("Requesting wakelock via nosleep.js polyfill.")
...
export async function requestWakeLock(uuid: string): Promise<boolean> {
	console.info(`requestWakeLock (${uuid})`)
	try {
		let visibilityListener = async () => {
			if (document.visibilityState === 'visible') {
				console.debug(`Visibility changed to visible. (${uuid})`);
				wakeLocksByUuid[uuid].wakeLockPromise = doRequestWakeLock(uuid);
			} else if (document.visibilityState === 'hidden') {
				console.debug(`Visibility changed to hidden. Releasing wakeLock. (${uuid})`);
		...
		console.error(`Could not acquire WakeLock! (${uuid})`, e)
		return false;
	}
}
...
async function doRequestWakeLock(uuid: string) {
	...
	console.info(`WakeLock acquired. (${uuid})`);
	wakeLock.addEventListener('release', (e) => {
		console.info(`WakeLock released. (${uuid})`);
	});
	return wakeLock;
}

export async function releaseWakeLock(uuid: string) {
	console.info(`releaseWakeLock (${uuid})`)
