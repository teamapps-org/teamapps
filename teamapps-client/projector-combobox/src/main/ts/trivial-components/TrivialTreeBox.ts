...
export class TrivialTreeBox<E> implements TrivialComponent {
	...
	private createEntryElement(entry: EntryWrapper<E>) {
		...
		const $entryAndExpanderWrapper = parseHtml('<div class="tr-tree-entry-and-expander-wrapper"></div>');
		$outerEntryWrapper.append($entryAndExpanderWrapper);
		($entryAndExpanderWrapper as any).trivialEntryWrapper = entry;
		...
		const $treeExpander = parseHtml('<div class="tr-tree-expander"></div>');
		$entryAndExpanderWrapper.append($treeExpander);
		let $entry = parseHtml(this.config.entryRenderingFunction(entry.entry, entry.depth));
	...
	public setSelectedEntryById(nodeId: number | string, reveal = false) {
	...
	public highlightTextMatches(searchString: string) {
		...
		for (let i = 0; i < this.entries.length; i++) {
			...
			const $entryElement = entry.$element.querySelector<HTMLElement>(':scope .tr-tree-entry');
	...
	setEnforceSingleExpandedPath(enforceSingleExpandedPath: boolean) {
		...
		let selectedEntry = this.findEntryById(this.selectedEntryId);
