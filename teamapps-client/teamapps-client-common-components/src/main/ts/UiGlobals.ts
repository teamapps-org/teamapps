...
export class UiGlobals {
	...
	public static readonly onGlobalKeyEventOccurred: TeamAppsEvent<UiGlobals_GlobalKeyEventOccurredEvent> = new TeamAppsEvent();
	public static readonly onNavigationStateChange: TeamAppsEvent<UiGlobals_NavigationStateChangeEvent> = new TeamAppsEvent();

	// dummy constructor
	constructor(config: UiComponentConfig, context: TeamAppsUiContext){
		throw new Error("UiGlobals should never be instantiated!");
	}
	...
	public static addRootComponent(containerElementId: string, component: UiComponent, context?: TeamAppsUiContext): void {
	...
	public static addClientToken(token: string) {
		let tokens = UiGlobals.loadClientTokens();
		tokens[token] = true;
		UiGlobals.saveClientTokens(tokens);
	}
	...
	public static getClientTokens(): string[] {
		return Object.keys(UiGlobals.loadClientTokens());
	}
	...
	public static createGenericErrorMessageShowable(title: string, message: string, showErrorIcon: boolean, options: UiGenericErrorMessageOption[], context: TeamAppsUiContext): Showable {
		let $div = parseHtml(`
        ...
        <div data-teamapps-id="ad352268-6d6b-473a-8c8d-316df8375361" style="width: 100%; height: 100%; background-color: white; margin: 0;">
            ...
            <div style="padding: 15px">
                    ...
                    <div style="display: flex; justify-content: space-around;">
                        ${options.map(o => `<div class="${UiGenericErrorMessageOption[o].toLowerCase()}" style="display: inline-block; text-align: center; cursor: pointer; padding: 5px; border-radius: 3px; user-select: none; border: 1px solid rgba(0, 0, 0, 0.09);">${UiGenericErrorMessageOption[o]}</div>`).join("")}
                    </div>
            </div>
        </div>
    </div>
...
`);
	...
	public static async pushHistoryState(relativeUrl: string) {
		...
		UiGlobals.onNavigationStateChange.fire({location: createUiLocation(), triggeredByUser: false});
	}
...
TeamAppsUiComponentRegistry.registerComponentClass("UiGlobals", UiGlobals);
...
let keyboardEventListener = (e: KeyboardEvent) => {
	...
	if (!modified && settings.unmodified
		...
		UiGlobals.onGlobalKeyEventOccurred.fire({
...
window.addEventListener('popstate', (event) => {
	UiGlobals.onNavigationStateChange.fire({
