...
public class PdfViewerTestHarness extends Panel {
    ...
    public PdfViewerTestHarness() {
        ...
        ToolbarButtonGroup buttonGroup = toolbar.addButtonGroup(
            new ToolbarButtonGroup()
        );

        ToolbarButton showPdfButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
                MaterialIcon.PICTURE_AS_PDF,
                "Show PDF",
                "Show the PDF document"
            )
        );
        ToolbarButton viewModeButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
                MaterialIcon.VIEW_STREAM,
                "View Mode",
                "Select page scrolling mode"
            )
        );
        ToolbarButton decreasePageButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
        ...
        ToolbarButton increasePageButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
        ...
        ToolbarButton zoomInButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
        ...
        ToolbarButton zoomOutButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
        ...
        ToolbarButton zoomToWidthButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
                MaterialIcon.BORDER_HORIZONTAL,
                "Zoom to width",
                "Auto-zoom to container width"
            )
        );
        ToolbarButton zoomToHeightButton = buttonGroup.addButton(
            ToolbarButton.createSmall(
                MaterialIcon.BORDER_VERTICAL,
                "Zoom to height",
                "Auto-zoom to container height"
            )
        );
        ...
        Runnable updatePageIndicator = () -> {
            String caption =
                maxPageNumber[0] > 0
                    ? currentPage[0] + " / " + maxPageNumber[0]
                    : String.valueOf(currentPage[0]);
            ToolbarButton indicator = new ToolbarButton(
                BaseTemplate.TOOLBAR_BUTTON_SMALL,
                new BaseTemplateRecord<>(
                    (org.teamapps.icons.Icon) null,
                    (String) null,
                    caption,
                    "Current page",
                    null
        ...
        String testPdfLink = getSessionContext().createResourceLink(
        ...
        showPdfButton.onClick.addListener(loadPdf::run);
        ...
        Button<BaseTemplateRecord> singlePageModeEntry = Button.create(
            MaterialIcon.VIEW_DAY,
            "Single Page Mode"
        );
        Button<BaseTemplateRecord> continuousModeEntry = Button.create(
            MaterialIcon.VIEW_STREAM,
            "Continuous Mode"
        );
        Button<BaseTemplateRecord> continuousVirtualModeEntry = Button.create(
            MaterialIcon.VIEW_STREAM,
            "Continuous Virtual"
        );
        ...
        viewModeButton
            .setDropDownComponent(viewModeMenu)
            .setDroDownPanelWidth(260);

        setTitle("PDF Viewer - Continuous Virtual");
        Consumer<UiPdfViewMode> applyViewMode = mode -> {
            pdfViewer.setViewMode(mode);
            singlePageControlsVisible[0] = mode == UiPdfViewMode.SINGLE_PAGE;
            updateSinglePageControlsVisibility.run();
            if (mode == UiPdfViewMode.SINGLE_PAGE) {
                setTitle("PDF Viewer - Single Page Mode");
            } else if (mode == UiPdfViewMode.CONTINUOUS) {
                setTitle("PDF Viewer - Continuous Mode");
            } else if (mode == UiPdfViewMode.CONTINUOUS_VIRTUAL) {
                setTitle("PDF Viewer - Continuous Virtual");
            }
        ...
        singlePageModeEntry.onClicked.addListener(() -> {
            applyViewMode.accept(UiPdfViewMode.SINGLE_PAGE);
            viewModeButton.closeDropDown();
        });

        continuousModeEntry.onClicked.addListener(() -> {
            applyViewMode.accept(UiPdfViewMode.CONTINUOUS);
            viewModeButton.closeDropDown();
        });

        continuousVirtualModeEntry.onClicked.addListener(() -> {
            applyViewMode.accept(UiPdfViewMode.CONTINUOUS_VIRTUAL);
            viewModeButton.closeDropDown();
        });
        ...
        pdfViewer.onPdfInitialized.addListener(initEvent -> {
            System.out.println(
                "PDF viewer rendered, max page number: " +
                    initEvent.getNumberOfPages()
            );
        ...
        pdfViewer.onZoomFactorAutoChanged.addListener(zoomFactorChangeEvent -> {
            System.out.println(
                "Zoom factor changed: " + zoomFactorChangeEvent.getZoomFactor()
            );
        });
    ...
    private String resolvePdfUrl(String url, String fallbackUrl) {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            return getSessionContext().createResourceLink(
                createPdfProxyResource(url)
            );
        }
        return url;
    }
