...
public class ZipRatioAnalyzer {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ZipRatioAnalyzer.class);

	public final String name;

	private final AtomicLong uncompressedSizeSum = new AtomicLong(0);
	private final AtomicLong statefullyCompressedSizeSum = new AtomicLong(0);
	private final AtomicLong statelesslyCompressedSizeSum = new AtomicLong(0);
	...
	public void addData(byte[] uncompressedData, int statefullyCompressedSize) {
		if (LOGGER.isDebugEnabled()) {
			...
			LOGGER.debug(name + ":: ?? uncompressed: " + FileUtils.byteCountToDisplaySize(uncompressedSum)
					+ "; ?? stateless: " + FileUtils.byteCountToDisplaySize(statelessSum) + " (r=" + ratio(uncompressedSum, statelessSum) + ")"
					+ "; ?? stateful: " + FileUtils.byteCountToDisplaySize(statefulSum) + " (r=" + ratio(uncompressedSum, statefulSum) + ")"
