...
public class ComboBoxTest extends AbstractFieldTest<ComboBox<?>> {
	...
	private ComboBox<SimpleTreeNodeImpl<Void>> field;
	...
	public ComboBoxTest(ComponentTestContext testContext) {
		super(testContext);
		model = new TestComboBoxModel(getTestContext());
	}

	@Override
	protected void addConfigurationFields(ResponsiveFormLayout responsiveFormLayout) {
		ConfigurationFieldGenerator<ComboBox<?>> fieldGenerator = new ConfigurationFieldGenerator<>(getComponent(), getTestContext());
		...
		responsiveFormLayout.addLabelAndField(MaterialIcon.HELP, "Show highlighting", fieldGenerator.createCheckBox("showHighlighting"));
		...
		CheckBox addToolButtonCheckBox = new CheckBox("add/removeToolButton");
		ToolButton toolButton = new ToolButton(MaterialIcon.LINK);
		addToolButtonCheckBox.onValueChanged.addListener(addToolButton -> {
			if (addToolButton) {
				getComponent().addToolButton(toolButton);
			} else {
				getComponent().removeToolButton(toolButton);
			}
		});
		responsiveFormLayout.addLabelAndField(MaterialIcon.LINK, "add/removeToolButton", addToolButtonCheckBox);
	...
	@Override
	protected ComboBox<SimpleTreeNodeImpl<Void>> createField() {
		field = new ComboBox<>(new TestComboBoxModel(getTestContext()));
	...
	public String getDocsHtmlResourceName() {
		return "org/teamapps/ux/testapp/docs/ComboBox.html";
	}
