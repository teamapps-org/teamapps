	...
	private updateNodes(animationDuration: number = 0): void {
		...
		nodeEnter.call(d3.drag()
			.on("start", function(event: any, d: any) {
				...
				if (that.lastDraggedNode) {
					that.lastDraggedNode.fx = null;
					that.lastDraggedNode.fy = null;
				}

				d.fx = d.x;
				d.fy = d.y;

				that.container.attr("cursor", "grabbing");
			})
			...
			.on("end", function (d: any) {
				...
				// d.fx = null;
				// d.fy = null;
				that.lastDraggedNode = d;
				that.container.attr("cursor", "grab")
			}), []
		);
		...
		foreignObjectInner
			...
			.html((d: DtoNetworkNode) => (d.template as Template).render(d.record.values));
