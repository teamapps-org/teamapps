...
export interface TrivialComboBoxConfig<E> {
	 ...
	 */
	textToEntryFunction?: TextToEntryFunction<E>,
...
export class TrivialComboBox<E> implements TrivialComponent {
	...
	setDropdownMinWidth(dropDownMinWidth: number | null) {
		this.$dropDown.style.minWidth = dropDownMinWidth != null ? (dropDownMinWidth + "px") : null;
	}

	setDropdownMaxHeight(dropDownMaxHeight: number | null) {
	...
	public openDropDown() {
		if (this.isDropDownNeeded()) {
			if (this.getMainDomElement().parentElement !== this.parentElement || this.dropdownAutoUpdateDisposable == null) {
				...
				this.dropdownAutoUpdateDisposable = this.dropdownAutoUpdateDisposable = positionDropdownWithAutoUpdate(this.$comboBox, this.$dropDown, {
	...
	public setTextToEntryFunction(textToEntryFunction: TextToEntryFunction<E>) {
		this.config.textToEntryFunction = textToEntryFunction;
	}

	public setToolButtons(buttons: ToolButton[]) {
