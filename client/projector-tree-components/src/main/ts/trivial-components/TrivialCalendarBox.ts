 ...
 * =========================LICENSE_END==================================
 */
import {type NavigationDirection, type TrivialComponent} from "./TrivialCore";
	...
	private updateDisplay() {
		...
		for (let w = 0; w < daysToBeDisplayed.length / 7; w++) {
			...
			for (let d = 0; d < 7; d++) {
				...
				const $td = parseHtml('<td>' + day.day + '</td>');
				...
				if (day.hasSame(this.selectedDate, 'day')) {
					...
					if (this.keyboardNavigationState === 'day' && this.keyboardNavCssClass) {
						$td.classList.add(this.keyboardNavCssClass);
					}
				}
		...
		if (this.config.highlightKeyboardNavigationState) {
			if (this.keyboardNavCssClass) {
				[this.$yearDisplay, this.$monthDisplay, this.$monthTable.querySelector(':scope td.' + this.keyboardNavCssClass)]
					.forEach(element => element.classList.remove(this.keyboardNavCssClass));
