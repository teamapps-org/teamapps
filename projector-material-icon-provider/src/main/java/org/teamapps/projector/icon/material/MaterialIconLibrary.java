package org.teamapps.projector.icon.material;
...
public class MaterialIconLibrary implements IconLibrary<MaterialIcon> {
	...
	private byte[] getSVG(String iconName, MaterialIconStyle style) {
		...
		try (InputStream inputStream = getClass().getResourceAsStream("/org/teamapps/projector/icon/material/" + style.getStyleType().getPackageName() + "/" + iconName)) {
			if (inputStream == null) {
				return null;
			}
